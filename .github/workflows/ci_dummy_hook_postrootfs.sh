...
imageref="${imageref##*://}"
imagetag="$(jq -r '."image-tag"' </usr/share/ublue-os/image-info.json)"

echo "ostreecontainer --url=$imageref:$imagetag --transport=containers-storage --no-signature-verification" | tee -a /usr/share/anaconda/interactive-defaults.ks
...
then
    echo ${flatpak_repo_gpg} | base64 -d > /var/lib/flatpak/repo/flathub.trustedkeys.gpg
else
